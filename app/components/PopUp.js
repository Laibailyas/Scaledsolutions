...
export default function PopUp({ videoIndexes = [], imageIndexes = [] }) {
    ...
    return (
        <>
            <section className="pop-success-stories">
                <div className="pop-success-container">

                    {/* Only selected video stories */}
                    {videoIndexes.map((i) => {
                        const story = videoStories[i];
                        return (
                            <div key={`video-${i}`} className="pop-story-block">
                                <div
                                    className="pop-story-video"
                                    onClick={() => setOpenVideo(videos[i])}
                                >
                                    <img
                                        src={`https://img.youtube.com/vi/${videos[i]}/hqdefault.jpg`}
                                        alt="Play Video"
                                        className="pop-video-thumbnail"
                                    />
                                    <div className="pop-play-icon">&#9658;</div>
                                </div>

                                <h3 className="pop-story-title">{story.title}</h3>
                                <div className="pop-story-flex">
                                    <div className="pop-story-box overview">
                                        <h4>Overview</h4>
                                        <p>{story.overview}</p>
                                    </div>
                                    <div className="pop-story-box quote">
                                        <p className="pop-quote-text">“{story.quote}”</p>
                                        <div className="pop-quote-author">
                                            <img
                                                src="https://cdn-icons-png.flaticon.com/512/847/847969.png"
                                                alt={story.author}
                                                className="pop-author-img"
                                            />
                                            <div>
                                                <h4>{story.author}</h4>
                                                <p>{story.role}</p>
                    ...
                    {imageIndexes.map((i) => {
                        ...
                        return (
                            <div key={`image-${i}`} className="pop-story-block">
                                <div className="pop-story-video">
                                    <img
                                        src={story.image}
                                        alt={story.title}
                                        className="pop-video-thumbnail"
                ...
                <div className="pop-pop-video-modal" onClick={() => setOpenVideo(null)}>
                    <div className="pop-video-wrapper" onClick={(e) => e.stopPropagation()}>
                        <iframe
                            ...
                            allowFullScreen
                        ></iframe>
                        <button className="pop-close-btn" onClick={() => setOpenVideo(null)}>
                            ×
                        </button>
                    </div>
                </div>
